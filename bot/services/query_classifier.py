...
class QueryClassifier:
    ...
    def __init__(self):
        ...
        self.about_patterns = [
            ...
            r'\b(производител\w+|завод\w+|офис\w+|контакт\w+|адрес\w+)\b',
            r'\b(история компании|о ecofes|об ecofes|о экофес)\b',
        ]
        
        # Расширенные технические паттерны на основе документации
        self.technical_patterns = [
            # Общие термины масел
            r'\b(мас\w+|смазк\w+|двигател\w+|мото\w+|вязкос\w+|fes|фес)\b',
            ...
            r'\b(кама|эскава\w+|мерсе\w+|бмв\w+|маз\w+|ямз|автоваз|умз|змз)\b',
        ...
        # Паттерны для каталога продуктов
        self.catalog_patterns = [
            r'\b(какие\s+(масла|продукт\w+)|что\s+есть|весь\s+ассортимент|список\s+масел)\b',
            r'\b(каталог|ассортимент|вся\s+продукц\w+|все\s+масла)\b',
            r'\b(что\s+производ\w+|какую\s+продукц\w+)\b',
            r'\b(виды\s+масел|типы\s+масел|линейка\s+масел)\b',
            r'\b(масла\s+(есть|ecofes|экофес))\b',
        ]
    ...
    def get_confidence_threshold(self, query_type: str) -> float:
        ...
        thresholds = {
            ...
            "catalog": 0.3,      # Используем RAG для каталога продуктов
